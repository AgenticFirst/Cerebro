...
interface FilesContextValue {
  ...
  updateBucket: (
    bucketId: string,
    patch: Partial<Pick<Bucket, 'name' | 'color' | 'icon' | 'isPinned'>>,
  ) => Promise<void>;
...
export function FilesProvider({ children }: { children: ReactNode }) {
  ...
  const defaultBucket = useMemo(() => buckets.find((b) => b.isDefault) ?? null, [buckets]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (sk === 'created' || sk === 'updated' || sk === 'name' || sk === 'opened')
        setSortKeyState(sk);
  ...
  const createBucket = useCallback<FilesContextValue['createBucket']>(
  ...
  const updateBucket = useCallback<FilesContextValue['updateBucket']>(
    async (bucketId, patch) => {
  ...
  const deleteBucket = useCallback<FilesContextValue['deleteBucket']>(
    async (bucketId, reassignTo) => {
  ...
  const uploadFiles = useCallback<FilesContextValue['uploadFiles']>(
    async (sourcePaths, bucketIdArg) => {
      ...
      if (created.length > 0) {
        addToast(
          `Saved ${created.length} file${created.length === 1 ? '' : 's'} to Files`,
          'success',
        );
  ...
  const saveExternalToFiles = useCallback<FilesContextValue['saveExternalToFiles']>(
    async (args) => {
  ...
  const moveItems = useCallback<FilesContextValue['moveItems']>(
    async (ids, bucketId) => {
  ...
  const copyItems = useCallback<FilesContextValue['copyItems']>(
    async (ids, bucketIdArg) => {
      const targetBucket = bucketIdArg ?? defaultBucket?.id;
      if (!targetBucket) {
  ...
  const softDelete = useCallback<FilesContextValue['softDelete']>(
    async (ids) => {
      await Promise.all(
        ids.map((id) => window.cerebro.invoke({ method: 'DELETE', path: `/files/items/${id}` })),
      );
      await Promise.all([refreshBuckets(), refreshItems()]);
      clearSelection();
    },
  ...
  const restore = useCallback<FilesContextValue['restore']>(
    async (ids) => {
  ...
  const hardDelete = useCallback<FilesContextValue['hardDelete']>(
    async (ids) => {
      const toUnlink = ids
