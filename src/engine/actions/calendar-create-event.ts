...
export function createCalendarCreateEventAction(deps: {
  ...
  return {
    ...
    chatExamples: [
      {
        en: 'Create a meeting with John tomorrow at 2pm for 30 minutes.',
        es: 'Crea una reunión con John mañana a las 2pm por 30 minutos.',
      },
      {
        en: 'Put "Dentist" on my calendar Friday at 9am.',
        es: 'Agenda "Dentista" el viernes a las 9am.',
      },
    ],
    availabilityCheck: () => {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        start: {
          type: 'string',
          description: 'Start time, ISO 8601 (e.g. 2026-06-03T14:00:00). Templated.',
        },
    ...
    execute: async (input: ActionInput): Promise<ActionOutput> => {
      ...
      if (!channel)
        throw new Error(
          'Calendar: Create Event — no calendar connected. Connect one in Integrations first.',
        );
      ...
      if (!res.ok) {
        ...
        return {
          data: { created: false, error: res.error ?? 'unknown' },
          summary: `Calendar create failed: ${res.error}`,
        };
      }
