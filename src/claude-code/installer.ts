...
**Default is delegation.** If a request is ambiguous — e.g. "the manual-writer should make a manual from this diagram" — that is **use**, not **change**. Edit an expert only when the user explicitly asks to modify the expert itself (prompt, persona, description). When in doubt, ask one short clarifier.

**Hard rule — never touch the expert files.** Files under \`.claude/agents/*.md\` are generated by the backend from the expert's database row. Do **not** \`Read\`, \`Edit\`, or \`Write\` them. If you find yourself about to open one to "show what the expert looks like" or to draft a rewrite, stop — that is never the right action. Use \`update-expert\` to change persona, \`attach-expert-context\` for permanent docs, and the \`Agent\` tool for per-task work.${teamsBlock}
...
`,
    },
    {
      ...
      description: 'Permanently attach a document to an existing expert as a template (output must follow this format) or reference (background knowledge). Use ONLY when the user says an expert should ALWAYS use a given file as guide/format — not for one-off per-task context.',
      body: `# Attach expert context

Use this skill when the user wants an existing expert to **always** use a specific document going forward — as a structural template ("siempre entrégame manuales en este formato", "use this as the template for every report") or as background reference ("ten siempre presente este documento", "always remember this brand guideline").

**Do not use this skill for one-off context.** If the user attached a diagram for THIS manual only, pass the file directly in your \`Agent\` call instead — the expert will read it for that single turn.
...
When in doubt, ask one short clarifier: "Should the expert *follow this file's format* in its output, or just *know what it says*?"

## Workflow

1. **Identify the expert.** Run \`list-experts\` if you don't already know the expert_id.
2. **Identify the file.** The user attached the file in the current message — its absolute path is in the user message as an \`@/absolute/path\` line (or in the conversation just above). Use that exact path.
3. **Confirm with the user in one sentence.** "I'll attach \`<filename>\` to \`<expert>\` as a \`<kind>\` — every future turn that expert will use it. Confirm?" Wait for a yes.
...
Where \`KIND\` is exactly \`template\` or \`reference\`.

## Result

- **SUCCESS** — tell the user it's saved and the expert will use it on every turn from now on. Do not also re-run the task in the same turn unless the user asked for it.
