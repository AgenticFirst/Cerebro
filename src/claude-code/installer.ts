...
5. **Deliver ONLY the file(s) the user asked for — nothing else.** End your reply with one literal \`@/absolute/path/to/file\` line per requested file, on the very last lines of the message, with nothing after them. Cerebro renders each trailing \`@/path\` as a clickable preview chip; without it, the file lives on disk but the user has no way to open it from chat.
   ...
   **Exact format:** each \`@/path\` line is the bare path and nothing else — no punctuation after the extension (\`@/a/b.docx.\` breaks the chip), no backticks/bold/quotes around it, no other words on the line, and it must be the very last line(s) of the message. If you already mentioned the path mid-message, still repeat it as the trailing \`@/path\` line.
`;
}
...
**Exact format:** each \`@/path\` line is the bare path and nothing else — no punctuation after the extension, no backticks/bold/quotes around it, no other words on the line — on the very last line(s) of the reply.`;
...
/**
 * Plain-language capability blurb for attached MCP servers. The model sees
 * the raw `mcp__<slug>__<tool>` names in its tool list already — this section
 * makes the persona narrate the capability correctly ("I can search your
 * Google Drive") instead of ignoring unfamiliar tool names.
 */
function renderExpertMcpSection(mcpGrants: McpGrantData[] = []): string {
  const lines: string[] = [];
  for (const grant of mcpGrants) {
    const toolNames = mcpToolNamesForGrant(grant);
    if (toolNames.length === 0) continue;
    const account = grant.server_account_label ? ` (${grant.server_account_label})` : '';
    const granted = grant.all_tools
      ? grant.server_tools
      : grant.server_tools.filter((t) => grant.selected_tools.includes(t.name));
    const toolList = granted
      .map(
        (t) =>
          `\`${mcpToolName(grant.server_slug, t.name)}\`${t.description ? ` — ${t.description}` : ''}`,
      )
      .join('\n  - ');
    lines.push(`- **${grant.server_name}**${account}:\n  - ${toolList}`);
  }
  if (lines.length === 0) return '';
  return (
    '\n## Connected tools\n\n' +
    'The user connected these services and granted you their tools. Use them directly whenever the task calls for it — no setup or permission step is needed:\n\n' +
    lines.join('\n') +
    '\n'
  );
}
...
function buildExpertBody(
  ...
  body += renderExpertMcpSection(mcpGrants);
...
| "What's the latest segment created in HubSpot?" / "¿Me traes el último segmento creado en HubSpot?" | \`hubspot_list_lists\` (\`sort_by: created_at\`) |
...
**MCP servers are set up in Settings, not from chat.** When the user asks to connect **Google Drive** or an **MCP server** ("connect Google Drive", "conecta Google Drive", "add an MCP server", "agrega un servidor MCP"), do NOT open a setup card — point them to **Settings → Integrations → MCP Servers**, where Google Drive has a guided connect flow and custom MCP servers (stdio or HTTP) can be added. After connecting, they can make a server available in chat (toggle) or grant it to a specific expert from the expert's panel ("MCP tools" section).
...
interface McpGrantData {
  ...
  server_account_label: string | null;
  server_tools: McpToolData[];
}

async function fetchExpertMcpGrants(
  backendPort: number,
  expertId: string,
): Promise<McpGrantData[]> {
  const result = await fetchJson<McpGrantData[]>(backendPort, `/experts/${expertId}/mcp-grants`);
  return result ?? [];
}
...
/** Connected MCP servers marked "available in chat" (for the main agent). */
async function fetchChatEnabledMcpServers(backendPort: number): Promise<McpServerData[]> {
  const result = await fetchJson<McpServerData[]>(backendPort, '/mcp-servers');
  return (result ?? []).filter((s) => s.chat_enabled);
}

/** `mcp__<slug>__<tool>` names an expert grant exposes in its frontmatter. */
function mcpToolNamesForGrant(grant: McpGrantData): string[] {
  const available = new Set(grant.server_tools.map((t) => t.name));
  const names = grant.all_tools
    ? grant.server_tools.map((t) => t.name)
    : grant.selected_tools.filter((n) => available.has(n));
  return names.map((n) => mcpToolName(grant.server_slug, n));
}
...
export async function installAll(options: InstallerOptions): Promise<void> {
  ...
  // Cerebro main agent (loads chat-enabled MCP server tools).
  const chatMcpServers = await fetchChatEnabledMcpServers(options.backendPort);
  installCerebroMainAgent(paths, teamsEnabled, chatMcpServers);
  ...
  // Fetch skills + context files + MCP grants for each regular expert
  // (teams don't carry any of them).
  const [regularSkillSets, regularContextSets, regularMcpGrantSets] = await Promise.all([
    ...
    Promise.all(regulars.map((expert) => fetchExpertMcpGrants(options.backendPort, expert.id))),
  ]);
...
export async function installExpert(options: InstallerOptions, expert: ExpertData): Promise<void> {
  ...
  if (expert.type === 'team') {
    ...
    const [skills, contextFiles, mcpGrants] = await Promise.all([
      fetchExpertSkills(options.backendPort, expert.id),
      fetchExpertContextFiles(options.backendPort, expert.id),
      fetchExpertMcpGrants(options.backendPort, expert.id),
    ]);
    writeExpertAgent(paths, expert, agentName, skills, contextFiles, mcpGrants);
  }
  index.experts[expert.id] = agentName;
  writeIndex(paths.indexPath, index);
}
...
function installCerebroMainAgent(
  paths: InstallerPaths,
  teamsEnabled: boolean,
  chatMcpServers: McpServerData[] = [],
): void {
  const memoryDir = path.join(paths.memoryRoot, 'cerebro');
  fs.mkdirSync(memoryDir, { recursive: true });

  // MCP servers marked "available in chat" expose their tools to the main
  // agent (per-server toggle in Settings → Integrations → MCP Servers).
  const mcpTools = chatMcpServers.flatMap((s) => s.tools.map((t) => mcpToolName(s.slug, t.name)));
  const file: AgentFile = {
    name: 'cerebro',
    description: "Cerebro: the user's personal AI assistant; coordinates with specialist experts.",
    tools: [...new Set([...CEREBRO_TOOLS, ...mcpTools])],
    body:
      buildCerebroBody(memoryDir, paths.skillsDir, teamsEnabled) +
      renderChatMcpSection(chatMcpServers),
  };
...
/** "Connected tools" blurb for the main agent (chat-enabled MCP servers). */
function renderChatMcpSection(servers: McpServerData[]): string {
  const lines = servers
    .filter((s) => s.tools.length > 0)
    .map((s) => {
      const account = s.account_label ? ` (${s.account_label})` : '';
      const toolList = s.tools
        .map(
          (t) => `\`${mcpToolName(s.slug, t.name)}\`${t.description ? ` — ${t.description}` : ''}`,
        )
        .join('\n  - ');
  ...
  return (
    '\n## Connected tools\n\n' +
    'The user connected these services in Settings → Integrations → MCP Servers. Their tools are available to you directly — use them whenever the task calls for it:\n\n' +
    lines.join('\n') +
    '\n'
  );
}
...
function writeExpertAgent(
  ...
  // Merge skill tool requirements + granted MCP tools with base expert tools.
  // MCP names stay listed even when the server is temporarily unhealthy: the
  // per-run --mcp-config just won't load that server, and an unmatched tool
  // name in frontmatter is inert — recovery then needs no resync.
  const skillTools = skills.flatMap((s) => s.tool_requirements ?? []);
  const mcpTools = mcpGrants.flatMap((g) => mcpToolNamesForGrant(g));
  const allTools = [...new Set([...EXPERT_TOOLS, ...skillTools, ...mcpTools])];

  const file: AgentFile = {
    name: agentName,
    description: expert.description || expert.name,
    tools: allTools,
    body: buildExpertBody(expert, memoryDir, skills, contextFiles, mcpGrants),
  };
  fs.writeFileSync(path.join(paths.agentsDir, `${agentName}.md`), renderAgentFile(file), 'utf-8');
  seedFileIfMissing(path.join(memoryDir, 'SOUL.md'), buildSoulFile(expert));
}
