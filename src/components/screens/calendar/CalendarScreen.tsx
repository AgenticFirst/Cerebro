...
import { CalendarDays, ChevronLeft, ChevronRight, RefreshCw, Plus, Plug, AlertTriangle, Sparkles, X, Loader2 } from 'lucide-react';
...
export default function CalendarScreen() {
  ...
  const [editing, setEditing] = useState<{ event?: CalendarEventDTO; start?: Date; end?: Date } | null>(null);
  const [connectOpen, setConnectOpen] = useState(false);
  const [ai, setAi] = useState<{ loading: boolean; text: string | null; error: string | null } | null>(null);
  ...
  const askAi = useCallback(async () => {
    ...
    setAi({ loading: false, text: res.ok ? res.text ?? '' : null, error: res.ok ? null : res.error ?? 'error' });
  ...
  const onKey = useCallback(
    (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        case 't': cal.goToday(); break;
        case 'w': cal.setViewMode('week'); break;
        case 'd': cal.setViewMode('day'); break;
        case 'r': void cal.refresh(); break;
        case 'c': setEditing({ start: roundedNow() }); break;
        case 'ArrowLeft': cal.goPrev(); break;
        case 'ArrowRight': cal.goNext(); break;
      }
    },
  ...
  const rangeLabel = useMemo(() => {
    ...
    if (cal.viewMode === 'day') {
      return cal.anchorDate.toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' });
    }
    const last = new Date(cal.windowStart);
    last.setDate(last.getDate() + 6);
    const sameMonth = last.getMonth() === cal.windowStart.getMonth();
    const startStr = cal.windowStart.toLocaleDateString(undefined, { month: 'short', day: 'numeric' });
    const endStr = last.toLocaleDateString(undefined, sameMonth ? { day: 'numeric', year: 'numeric' } : { month: 'short', day: 'numeric', year: 'numeric' });
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 pt-5 pb-3">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-[18px] font-semibold text-text-primary leading-tight">{t('calendar.title')}</h1>
            <p className="text-[12px] text-text-tertiary">{rangeLabel}</p>
          </div>

          {/* Controls */}
          <div className="flex items-center gap-1.5">
            <button onClick={cal.goToday} className="px-2.5 py-1 rounded-md text-[12px] text-text-secondary hover:bg-bg-hover">
              {t('calendar.today')}
            </button>
            <button onClick={cal.goPrev} className="p-1 rounded-md text-text-secondary hover:bg-bg-hover"><ChevronLeft size={16} /></button>
            <button onClick={cal.goNext} className="p-1 rounded-md text-text-secondary hover:bg-bg-hover"><ChevronRight size={16} /></button>
              ...
              {(['day', 'week', 'month'] as const).map((m) => (
                <button
                  ...
                  className={clsx(
                    ...
                    cal.viewMode === m ? 'bg-accent/15 text-accent' : 'text-text-secondary hover:bg-bg-hover',
                  )}
                >
                  {t(`calendar.views.${m}`)}
                </button>
        ...
        {expiredAccounts.map((a) => (
          <Banner key={a.id} tone="warn" text={t('calendar.errors.tokenExpired', { email: a.email })} />
        ))}
        {erroredAccounts.map((a) => (
          <Banner key={a.id} tone="error" text={t('calendar.errors.syncFailed', { email: a.email, error: a.last_error ?? '' })} />
        ))}
      </div>
        ...
        <div className="h-full rounded-xl border border-border-subtle bg-bg-surface/30 overflow-hidden">
          ...
          {cal.viewMode === 'month' ? (
            <CalendarMonthView
              ...
              onPickDay={(day) => { cal.goToDate(day); cal.setViewMode('day'); }}
      ...
      {connectOpen && (
        <Suspense fallback={null}>
          <CalendarConnectModal onClose={() => setConnectOpen(false)} onPersisted={() => void cal.reloadAccounts()} />
        </Suspense>
      )}

      {ai && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setAi(null)}>
          <div
          ...
          >
            <div className="flex items-center gap-2 px-4 py-3 border-b border-border-subtle">
              ...
              <h2 className="flex-1 text-[14px] font-semibold text-text-primary">{t('calendar.ai.weekSummary')}</h2>
              <button onClick={() => setAi(null)} className="text-text-tertiary hover:text-text-secondary"><X size={16} /></button>
            </div>
