...
function fmtRange(startMs: number, endMs: number, allDay: boolean): string {
  ...
  if (allDay) return s.toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      ...
      >
        ...
        <div className="px-4 py-3 space-y-2 text-[13px] text-text-secondary">
          ...
          {ev.description && (
            <p className="text-[12px] text-text-tertiary whitespace-pre-wrap line-clamp-6">{ev.description}</p>
          )}
          <div className="flex items-center gap-2 pt-1 text-[11px] text-text-tertiary">
            <span>{ev.transparency === 'transparent' ? t('calendar.free') : t('calendar.busy')}</span>
            {ev.visibility === 'private' && (
              <span className="flex items-center gap-0.5"><Lock size={10} /> {t('calendar.private')}</span>
            )}
            {ev.conflict != null && (
        ...
        {isInvite && (
          <div className="flex items-center gap-2 px-4 pb-2">
            ...
            <RsvpButton label={t('calendar.event.rsvpYes')} active={ev.rsvp_status === 'accepted'} onClick={() => doRsvp('accepted')} disabled={busy} />
            <RsvpButton label={t('calendar.event.rsvpMaybe')} active={ev.rsvp_status === 'tentative'} onClick={() => doRsvp('tentative')} disabled={busy} />
            <RsvpButton label={t('calendar.event.rsvpNo')} active={ev.rsvp_status === 'declined'} onClick={() => doRsvp('declined')} disabled={busy} />
          </div>
        )}
    ...
    {confirmDelete && (
      <AlertModal
        ...
        actions={[
          ...
          { label: t('calendar.event.delete'), primary: true, variant: 'danger', onClick: () => void doDelete() },
        ]}
      />
    )}
    </>
