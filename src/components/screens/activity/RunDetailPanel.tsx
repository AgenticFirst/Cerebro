...
export default function RunDetailPanel({
  ...
  const fetchDetail = useCallback(
    async (id: string, signal: { cancelled: boolean }, opts?: { silent?: boolean }) => {
      ...
      try {
        const [runRes, eventsRes, childrenRes] = await Promise.allSettled([
          ...
          window.cerebro.invoke<EventRecord[]>({
            method: 'GET',
            path: `/engine/runs/${id}/events?limit=500`,
          }),
          window.cerebro.invoke<RunListResponse>({
            method: 'GET',
            path: `/engine/runs/${id}/children`,
          }),
        ]);
  ...
  const displayName =
    routineName ??
    (run
      ? t(`activity.filter.${run.run_type}`, { defaultValue: run.run_type })
      : t('activity.run'));
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin px-5 py-5 space-y-6">
        {loading ? (
          ...
          <>
                ...
                <div className="space-y-1 text-[11px] text-text-tertiary">
                  <div>
                    ...
                    <span className="text-text-secondary">
                      {formatTimestamp(run.started_at, t)}
                    </span>
                  </div>
                  <div>
                    {t('activity.finished')}:{' '}
                    <span className="text-text-secondary">
                      {formatTimestamp(run.completed_at, t)}
                    </span>
                  </div>
                </div>
            ...
            {activeTab === 'logs' && <RunLogs run={run} events={events} />}
