...
- **Approvals gate every external-facing action.** `run-chat-action` pauses for human approval by default. The one exception is a user-set, per-destination "don't ask again" rule (today: a specific Slack channel) — stored in `auto_approval_rules`, managed via the `manage-auto-approvals` skill. There is no global skip. Routine steps that touch Telegram/HubSpot/WhatsApp/email should set `requiresApproval: true`.
